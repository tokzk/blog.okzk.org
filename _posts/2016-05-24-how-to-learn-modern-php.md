...
最近は、GitHubで「awesome ◯◯」で調べると、◯◯関連の情報をまとめたドキュメントなどがあるのでそれも参考にする。

そして見つけたのがこれ。

[PHP: The Right Way](http://www.phptherightway.com)

ここを見とけば、一通りはOKなのではないだろうか。
このなかで、適用できそうなものをどんどん採用していくことにする。
...
以前はXAMPPとかローカルのApacheなどがありました。Ruby on Railsでpowを使ってた場合などに併用に苦労した覚えがありますが、現在はビルトインウェブサーバーがあるようなので、それを使用することにします。
...
ライブラリの依存性管理は、最近は`PEAR`は使わず、`Composer`というもので、venderディレクトリ以下にインストールするのが一般的だそうで、RubyでいうところのBundlerのようなもののようです。
今回はPHPUnitとSeleniumを簡単に使えるFacebookのライブラリphp-webdriverを使って、E2Eのテストだけ書いておくことに。
ここらへんは長くなりそうなので別途要望があれば書く。
...
まずは、[jwagejwage/SplClassLoader.php](https://gist.github.com/jwage/221634) でクラスローダーをダウンロードしてきて、srcディレクトリに配置する。

`./public_html/phptest.php`というファイルを用意して、以下のように記述し、SplClassLoader.phpを読み込む。
